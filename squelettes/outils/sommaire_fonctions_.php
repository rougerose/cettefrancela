...
function sommaire_d_une_page(&$texte, &$nbh3, $page=0, $num_pages=0) {
	...
	if(!defined('_CS_PRINT')) {
		...
		$haut = "<a title=\"$titre\" href=\"".$self."#sommaire\" class=\"sommaire_ancre\">&nbsp;</a>"; // modif cld
	} else $haut = '';
	...
	for($i=0;$i<count($regs[0]);$i++,$index++){
		$ancre = " id=\"sommaire_$index\">"; // modif cld
		if (($pos2 = strpos($texte, $regs[0][$i], $pos))!==false) {
			...
			$sommaire .= "<dd><a $st title=\"$titre\" href=\"{$artpage}#sommaire_$index\">$lien</a>$p</dd>"; // modif cld
		}
	}
	return $sommaire;
}
